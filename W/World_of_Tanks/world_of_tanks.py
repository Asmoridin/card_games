...
"""
Collection manager/purchase suggester for a World of Tanks. Reads in card data and deck lists,
tracks collection ownership, and suggests cards to acquire based on missing cards in decks and
overall collection needs. Also tracks win-loss records by various categories for strategic insights
"""
...
FILE_PREFIX = os.path.join("card_games", "W", "World_of_Tanks")
...
if os.getcwd().endswith('card_games'):
    FILE_PREFIX = os.path.join("W", "World_of_Tanks")
...
# Pull in card data
with open(os.path.join(DATA_PREFIX, "World of Tanks Data.txt"), 'r', encoding="UTF-8") as file_h:
    lines = file_h.readlines()
lines = [line.strip() for line in lines]

# Pull in additional data files, and set up data structures
with open(os.path.join(DATA_PREFIX, "World of Tanks Sets.txt"), 'r', encoding="UTF-8") as in_sets:
    game_sets = in_sets.readlines()
game_sets = [line.strip() for line in game_sets if line.strip() != '']

with open(os.path.join(DATA_PREFIX, "World of Tanks WL Data.txt"), 'r', encoding="UTF-8") as in_wl:
...
for line in lines:
    ...
    if line_vals[2] == 'Base' and len(line_vals) == 7:
        card_name, card_country, card_type, card_sets, card_number, card_rarities, \
            card_own = line_vals
    elif len(line_vals) == 12:
        card_name, card_country, card_type, card_atk, card_ammo, card_hp, card_vp, \
            card_abilities, card_sets, card_number, card_rarities, card_own = line_vals
    else:
        print("Line with wrong number of items:")
        print(line)
        continue
    ...
    if card_country not in valid_countries:
        print(f"Invalid card country {card_country} for {card_name}")
    ...
    item_list.append((card_name, card_country, card_type, card_sets, card_rarities, \
        card_own, CARD_MAX))
...
if __name__ == "__main__":
    out_file_h = open(FILE_PREFIX + "/World of Tanks Out.txt", 'w', encoding="UTF-8")

    double_print("World of Tanks Inventory Tracker Tool\n", out_file_h)
