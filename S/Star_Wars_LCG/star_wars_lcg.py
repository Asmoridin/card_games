...
def get_index(in_deck):
    ...
    if in_deck['Affiliation'] != "" and in_deck['Affiliation'] not in VALID_AFFILIATIONS:
        if in_deck['Affiliation'] in ['Mercenary Contacts', 'Expendable Allies',
                ...
                'Desperate Allies', 'Mercenary Contacts', 'Fighters for Freedom',
                "Information Network"]:
            ret_index = 6
        else:
...
def read_deck(in_deck_lines, deck_name):
    ...
    for this_deck_line in in_deck_lines:
        ...
        if this_deck_line == "Affiliation: Promise of Power":
            ...
            continue
        if this_deck_line == "Affiliation: Imperial Contractors":
            ret_deck['Affiliation'] = "Imperial Contractors"
            continue
        if this_deck_line == "Affiliation: Dark Masters":
            ret_deck['Affiliation'] = "Dark Masters"
            continue
        if this_deck_line == "Affiliation: No Questions Asked":
            ret_deck['Affiliation'] = "No Questions Asked"
            continue
        ...
        if this_deck_line == "Affiliation: Mercenary Contacts":
            ret_deck['Affiliation'] = "Mercenary Contacts"
            continue
        if this_deck_line == "Affiliation: Fighters for Freedom":
            ret_deck['Affiliation'] = "Fighters for Freedom"
            continue
        if this_deck_line == "Affiliation: Information Network":
            ret_deck['Affiliation'] = "Information Network"
            continue
        try:
